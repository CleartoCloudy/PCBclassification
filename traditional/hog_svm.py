...
from traditional.features import extract_hog_features, load_dataset_from_directory

np.random.seed(RANDOM_SEED)


def train_hog_svm(train_dir, val_dir=None):
    """
    训练HOG+SVM模型
    返回: model, scaler, train_metrics, val_metrics
    """
    print("\n" + "=" * 60)
    print("实验一: HOG + SVM")
    print("=" * 60)
    ...
    print("\n[1/3] 提取HOG特征...")
    t0 = time.time()
    X_train, y_train, _ = load_dataset_from_directory(train_dir, extract_hog_features, (IMG_SIZE_TRAD, IMG_SIZE_TRAD))
    ...
    if val_dir and os.path.exists(val_dir):
        ...
        X_val, y_val, _ = load_dataset_from_directory(
            val_dir, extract_hog_features, (IMG_SIZE_TRAD, IMG_SIZE_TRAD))
    ...
    if val_dir and os.path.exists(val_dir):
        print("\n[3/3] 验证集评估...")
        X_val, y_val, _ = load_dataset_from_directory(val_dir, extract_hog_features, (IMG_SIZE_TRAD, IMG_SIZE_TRAD))
        X_val_scaled = scaler.transform(X_val)
        ...
        val_metrics = {"accuracy": 0, "f1": 0, "confusion_matrix": None, "y_true": None, "y_pred": None}

    # 4. 保存模型
    model_path = os.path.join(MODELS_DIR, "hog_svm.pkl")
    ...
    # 保存训练信息供报告使用
    import json
    hist_path = os.path.join(LOGS_DIR, "hog_svm_history.json")
...
def test_hog_svm(test_dir, model_path=None):
    """测试HOG+SVM模型"""
    ...
    X_test, y_test, _ = load_dataset_from_directory(test_dir, extract_hog_features, (IMG_SIZE_TRAD, IMG_SIZE_TRAD))
...
def main():
    ...
    train_hog_svm(train_dir, val_dir)
